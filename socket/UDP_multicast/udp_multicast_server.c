...
void handle_udp_msg(int fd)
{
	...
	while (1)
	{
		...
		printf("server:%s\n",buf);  //打印自己发送的信息给
		sendto(fd, buf, BUFF_LEN, 0, (struct sockaddr*)&client_addr, len);  //发送信息给client，注意使用了clent_addr结构体指针
	}
}
 ...
 */
 ...
 int main(int argc, char* argv[])
 {
	...
	ser_addr.sin_addr.s_addr = htonl(INADDR_ANY); //IP地址，需要进行网络序转换，
	...
	//set multicast loop
	SetMulticastLoop(server_fd);

	//set socket to add multicast address
	AddSocketToMulticast(server_fd, MULTICAST);
